...
const cards = (a) =>
  `<div class="grid">${a.map((x, i) => `<article class="card reveal">${x.ai ? `<div class="service-photo"><img loading="lazy" decoding="async" src="${x.ai}" alt="${x.name} 서비스 이미지"></div>` : ""}<span class="card-icon">${icons[i % icons.length]}</span><span class="num">${String(i + 1).padStart(2, "0")}</span><h3>${x.name}</h3><p>${x.desc}</p>${x.detail ? `<button class="more card-detail-trigger" type="button" data-detail-title="${E(x.name)}" data-detail-description="${E(x.detail)}" data-detail-image="${E(x.image)}">사진과 설명 보기 →</button>` : `<a class="more" href="${x.href}">자세히 보기 →</a>`}</article>`).join("")}</div>`;
const serviceCards = () => [
...
function home(r = null) {
  ...
  return (
    ...
    `<section class="soft"><div class="wrap article"><article class="article-main"><span class="kicker">DAEJEON VIRAL MARKETING GUIDE</span><h2>대전바이럴마케팅, 채널보다 먼저 정할 것</h2><p>대전바이럴마케팅은 지역명을 글에 반복하는 것보다, 실제 고객이 어떤 상황에서 검색하고 무엇을 비교하는지 정리하는 데서 시작합니다. 둔산·유성·대덕처럼 생활권과 업종 경쟁이 다른 만큼 같은 문구를 여러 곳에 복사하기보다 서비스와 고객 질문에 맞는 콘텐츠를 만들어야 합니다.</p><h3>검색·블로그·카페·SNS의 역할을 나눕니다</h3><p>블로그는 검색 질문에 답하고, 카페는 지역 생활 정보와 신뢰를 쌓고, SNS는 브랜드의 실제 모습과 소식을 보여주는 채널입니다. 홈페이지는 각 채널에서 들어온 고객이 서비스 범위와 진행 방식, 상담 방법을 확인하는 기준점이 됩니다.</p><h3>성과는 노출만 보지 않습니다</h3><p>대전온라인마케팅의 성과는 검색 노출뿐 아니라 유입된 페이지, 전화·카카오톡 문의, 상담으로 이어진 콘텐츠를 함께 확인해야 합니다. 먼저 핵심 서비스를 정하고 제목·본문·내부 링크를 정리한 뒤 반응이 좋은 주제를 확장하는 방식으로 개선합니다.</p></article><aside class="aside"><strong>대전바이럴마케팅 구성</strong><a href="${U("blog-marketing.html")}">대전 블로그 콘텐츠</a><a href="${U("cafe-marketing.html")}">지역 카페 마케팅</a><a href="${U("sns-marketing.html")}">SNS 콘텐츠 운영</a><a href="${U("seo-marketing.html")}">검색 구조 최적화</a><a href="${U("daejeon-construction-marketing.html")}">대전 업종별 사례</a></aside></div></section>` +
...
function service(s) {
  ...
  return (
    ...
    `<section class="hero service-hero"><div class="wrap"><span class="kicker">MARKETING SERVICE</span><h1><em>${s.name}</em>,<br>보여주기보다 문의까지 생각합니다.</h1><p class="lead">${s.desc} 처음부터 끝까지 1인 프리랜서가 직접 진행합니다.</p></div></section><section><div class="wrap">${cards(["현황 진단", "방향 설계", "제작과 운영"].map((name, i) => ({ name, desc: ["현재 채널과 경쟁 환경을 확인합니다.", "상담으로 이어지는 순서를 정리합니다.", "직접 만들고 결과를 보완합니다."][i], detail: [`${s.name} 검색 결과와 현재 운영 채널, 경쟁 브랜드의 콘텐츠와 문의 동선을 함께 살펴봅니다. 확인한 자료를 바탕으로 먼저 개선할 지점을 정리합니다.`, `${s.name} 고객이 정보를 찾고 비교한 뒤 상담하는 과정을 기준으로 콘텐츠 순서와 채널 역할을 설계합니다. 예산과 일정에 맞는 실행 범위도 함께 정합니다.`, `기획한 방향에 맞춰 콘텐츠와 화면을 직접 제작하고 운영합니다. 노출과 반응, 실제 문의 흐름을 확인해 제목·이미지·내용을 계속 보완합니다.`][i], image: `assets/${imgs[i % imgs.length]}` })))}</div></section><section class="service-work soft"><div class="wrap"><div class="center-head"><h2>${s.name} Portfolio</h2><p>${s.name} 분야에서 직접 진행한 작업만 모았습니다.</p></div><div class="photo-grid work-gallery service-gallery">${imgs
      .map(
        (a, i) =>
...
pages.forEach((v, k) =>
  writeFileSync(
    k,
    v.replaceAll("업체", "브랜드").replaceAll("직접 만든 작업", "작업 결과물").replaceAll("처음부터 남다르게", "시작부터 남다르게").replace(/href="([^"#]+)\.html(#[^"]*)?"/g, 'href="/$1$2"').replace("</head>", `<link rel="stylesheet" href="premium.css?v=${V}"></head>`).replace("</body>", `<script src="motion-enhance.js?v=${V}" defer></script></body>`),
  ),
);
